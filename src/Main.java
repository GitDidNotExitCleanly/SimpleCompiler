 ...
 * lexing errors.
 * 
 * * The Parser must have a constructor which accepts a Tokeniser, furthermore
 * Parser must provide a public method getErrorCount which returns the total
 * number of parsing errors. In particular, the Parser must have a public method
 * parse which returns an AST.
 * 
 * * ASTPrinter must provide a constructor which accepts a PrintWriter, and it
 * must also implement the visitor interface.
 * 
 * * SemanticAnalyzer must provide a nullary constructor and a public method
 * analyze which accepts an AST, and returns the total number of semantic
 * errors.
 */
public class Main {
	...
	public static void main(String[] args) {
		...
		if (mode == Mode.LEXER) {
			...
			// write to file
			try {
				File file = new File("out/Main.class");
				FileOutputStream fout = new FileOutputStream(file);
				fout.write(codegen.data);
				fout.close();
			} catch (IOException e) {
				System.out.println("File out/Main.class does not exist.");
				System.exit(FILE_NOT_FOUND);
				return;
			}
